...
FFMPEG_AUDIO_IDX=$(echo "$DEVICE_LIST" | grep -A 99 "AVFoundation audio devices" | grep -vE "Teams|Zoom|BlackHole|Immersed|Virtual|Aggregate|Display|Soundflower" | grep -m 1 "\[" | sed 's/.*\[\([0-9]*\)\].*/\1/')
set -e
