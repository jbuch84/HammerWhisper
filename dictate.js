...
if (process.argv.includes("--check")) {
    ...
    const keyValid = typeof apiKey === "string" && apiKey.startsWith("gsk_") && apiKey !== "YOUR_API_KEY_HERE";
    check("API key present in config", keyValid, "Open ~/quickgroq/config.json and paste your Groq key (starts with gsk_)");

    // 3. Node.js version (native fetch requires v18+)
    const nodeMajor = parseInt(process.version.replace("v", "").split(".")[0]);
    check(`Node.js version OK (${process.version})`, nodeMajor >= 18, "Update Node.js: brew upgrade node  OR  winget upgrade OpenJS.NodeJS");
    ...
    console.log("");
    console.log(allGood
        ? "Everything looks good! Press your hotkey to start dictating."
        : "Fix the items above and run: node dictate.js --check"
    );
